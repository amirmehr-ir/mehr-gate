...
class MehrGate
{
	...
	public function request( $Amount, $Description = "", $Mobile = "" )
	{
		$MerchantID  = config( 'mehr-gate.gate.test' ) ? "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" : config( 'mehr-gate.gate.token' );
		$SandBox     = config( 'mehr-gate.gate.test' );
		$CallbackURL = config( 'mehr-gate.gate.callback' );
		$Email       = "";
		$ZarinGate   = false;
